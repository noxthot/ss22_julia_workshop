...
mach_standardizer = machine(standardizer_model, X_test)

# Fit model (compute mean and std)
fit!(mach_standardizer)

# Transform the data by applying the fitted model
X_test_trans = MLJ.transform(mach_standardizer, X_test)

# X_test_trans unfortunately contains NaNs. We replace them by 0.
X_test_trans .= ifelse.(isnan.(X_test_trans), 0, X_test_trans)
```
...
mach_pca = machine(pca, X_test_trans)
fit!(mach_pca)
```
...
```julia
X_test_pca = MLJ.transform(mach_pca, X_test_trans)
...
1. `X_test_trans` is of type `DataFrame` which is not supported by `UMAP`. Convert the dataframe into a `Matrix`.
...
X_test_umap_mat = umap(Matrix(X_test_trans)', 2)
